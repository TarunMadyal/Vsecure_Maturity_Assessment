 ...
 *
 * Control areas are clubbed under their IAM domain (IGA / PAM / WAM / CIAM),
 * each tinted with its own subtle accent colour so the sections read as
 * distinct families. A visible domain heading appears whenever the assessment
 * spans more than one domain (the Full IAM assessment); single-domain
 * assessments stay flat.
 */
export default function ControlAreaNav({ areas, answers, currentAreaId, onSelect }) {
  ...
  return (
    <>
      ...
      <nav className="hidden lg:block w-72 flex-none no-print" aria-label="Control areas">
        <div className="sticky top-20 rounded-2xl border border-edge bg-card p-3 max-h-[calc(100vh-6rem)] overflow-y-auto">
          {groups.map((group, gi) => {
            ...
            return (
              <div key={group.type} className={gi > 0 ? 'mt-4 pt-4 border-t border-edge' : ''}>
                <div className="px-2 pb-2">
                  {multiDomain ? (
                    <>
                      <span className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider" style={{ color: t.accent }}>
                        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: t.accent }} aria-hidden="true" />
                        {group.type}
                      </span>
                      <span className="block mt-0.5 text-[11px] font-medium text-ink-3">
                        {group.name}
                      </span>
                    </>
                  ) : (
      ...
      {/* Mobile horizontal scroller with domain dividers */}
