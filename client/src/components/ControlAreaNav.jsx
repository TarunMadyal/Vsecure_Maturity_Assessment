...
import { groupByAreaType } from '../lib/theme';
 ...
 * with a visible domain heading whenever the assessment spans more than one
 * domain (the Full IAM assessment). Single-domain assessments stay flat.
 */
            ...
            <div key={group.type} className={gi > 0 ? 'mt-4 pt-4 border-t border-edge' : ''}>
              <p className="px-2 pb-2 text-[11px] font-bold uppercase tracking-wider text-accent">
                {multiDomain ? (
                  <>
                    {group.type}
                    <span className="block font-medium normal-case tracking-normal text-ink-3">
                      {group.name}
                    </span>
                  </>
                ) : (
        ...
        <div className="flex items-center gap-1.5 w-max">
          {groups.map((group) => (
            <div key={group.type} className="flex items-center gap-1.5">
              {multiDomain && (
                <span
                  className="flex-none text-[10px] font-bold uppercase tracking-wider text-white btn-gradient rounded-md px-2 py-1"
                  aria-hidden="true"
                >
                  {group.type}
                </span>
